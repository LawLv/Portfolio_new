...
class NNAgent:
    ...
    def __set_loss_function(self):
        ...
        def my_lossfunction1():
            return 1/(tf.reduce_sum(self.pv_vector)-0.1*tf.math.reduce_std(self.pv_vector))
        def my_lossfunction2():
            # return 1/(tf.reduce_sum(self.pv_vector)-0.01*tf.minimum(self.pv_vector,0))
            return 1/(tf.reduce_sum(self.pv_vector)-0.1*tf.math.reduce_std(tf.minimum(self.pv_vector,0)))
        ...
        elif self.__config["training"]["loss_function"] == "my_lossfunction1":
            loss_function = my_lossfunction1
        elif self.__config["training"]["loss_function"] == "my_lossfunction2":
            loss_function = my_lossfunction2
