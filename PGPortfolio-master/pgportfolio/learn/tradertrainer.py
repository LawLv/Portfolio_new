...
class TraderTrainer:
    ...
    def __log_result_csv(self, index, time):
        ...
        result = Result(test_pv=[v_pv],
                        ...
                        # test_history=[','.join(str(e).strip() for e in benefit_array)],
                        config=[json.dumps(self.config)],
                        net_dir=[index],
                        backtest_test_pv=[backtest.test_pv],
                        backtest_test_history=[''.join(str(e)+', ' for e in backtest.test_pc_vector)],
                        # backtest_test_history=[','.join(str(e).strip() for e in backtest.test_pc_vector)],
        ...
        # new_data_frame = pd.DataFrame.from_records([dict(r)._asdict() for r in result]).set_index("net_dir")
        
        _result = result._asdict()
        # print(_result)
        new_data_frame = pd.DataFrame.from_dict(_result,dtype=str)
        new_data_frame = new_data_frame.set_index("net_dir")
